...
export default class SequenceStep {
  ...
  generateFilter(): string {
    ...
    const videoList = this.mediaList.filter(media => media.hasVideo || (media.hasAudio && !media.hasVideo && !this.mediaList.some(other => other.hasVideo && media.user && other.user?.id === media.user?.id)));
